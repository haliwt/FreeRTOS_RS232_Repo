  ...
  printf("KEY1、KEY2和KEY3对应不同任务情况\n");
    ...
    while(1)
    {
      ...
      {         
		...
		//HAL_UART_Receive(&husart_debug,aRxBuffer,1,0xfff);  
	   MSG_T   *ptMsg;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *UartHandle)
{
 // HAL_UART_Transmit(&husart_debug,&aRxBuffer,1,0);
  ptMsg-> ptMsg->ucMessageID=aRxBuffer[0];
  HAL_UART_Receive_IT(&husart_debug,&aRxBuffer,1);
  xResult = xQueueReceive(xQueue2,                   /* 消息队列句柄 */
                            (void *)&ptMsg,  		   /* 这里获取的是结构体的地址 */
                            (TickType_t)xMaxBlockTime);/* 设置阻塞时间 */
...
portBASE_TYPE xHigherPriorityTaskWoken = pdFALSE;
    u8 cChar;
     /*????,????????*/
    if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET && rec_f==0)
    {    
        cChar = USART_ReceiveData(USART1);
        xQueueSendToBackFromISR(xQueueRx, &cChar, &xHigherPriorityTaskWoken); /*???????TASK?????,ze xHigherPriorityTaskWoken?true*/
        portEND_SWITCHING_ISR( xHigherPriorityTaskWoken );
    }
#endif